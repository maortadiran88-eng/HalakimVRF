...
function TechRequestBox({loginRole}) {
  ...
  const send = async () => {
    ...
    setLoading(true);
    try {
      await fbSaveTechRequest({ text: text.trim(), submittedBy: name.trim()||'אנונימי', role: loginRole });
  ...
  return (
    <div style={{background:'var(--card)',borderRadius:12,padding:'16px 18px',boxShadow:'0 1px 4px var(--shadow)',border:'1px solid #e3f2fd',marginBottom:18}}>
      <div style={{fontWeight:'bold',fontSize:14,color:'#1565c0',marginBottom:10}}>💬 חסר לך דגם במערכת?</div>
      ...
      {sent
        ...
        : <>
          <input value={name} onChange={e=>setName(e.target.value)} placeholder="שמך (אופציונלי)"
            style={{...INS, marginBottom:8, fontSize:13, padding:'8px 12px'}}/>
          ...
          <button onClick={send} disabled={loading} style={{...BPr('#1565c0'),width:'100%'}}>
...
// ══════════ HOME SCREEN ══════════
function HomeScreen({data, onNav, recent, favorites, onToggleFav, loginRole}) {
  ...
  const recentModels = recent.slice(0,6).map(rv => {
    const b = data.brands.find(x => x.id===rv.bid);
    const c = b?.categories.find(x => x.id===rv.cid);
    const m = c?.models.find(x => x.id===rv.mid);
    if (!b||!c||!m) return null;
    return {b,c,m,ts:rv.ts};
  }).filter(Boolean);

  const favModels = [];
  data.brands.forEach(b => b.categories.forEach(c => c.models.forEach(m => {
    if (favorites.has(m.id)) favModels.push({b,c,m});
  })));
        ...
        <div style={{marginBottom:18}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(148px,1fr))',gap:8}}>
            {recentModels.map(({b,c,m,ts}) => (
              <div key={m.id} onClick={() => onNav(b.id,c.id,m.id)}
                style={{background:'var(--card)',borderRadius:10,padding:'11px 13px',cursor:'pointer',boxShadow:'0 1px 4px var(--shadow)',borderRight:`4px solid ${b.color}`,transition:'all .15s'}}
                onMouseEnter={e=>e.currentTarget.style.transform='translateY(-2px)'}
                onMouseLeave={e=>e.currentTarget.style.transform=''}>
        ...
        return (
          <div key={b.id} style={{marginBottom:8,background:'var(--card)',borderRadius:12,overflow:'hidden',boxShadow:'0 1px 4px var(--shadow)'}}>
            <div onClick={() => setExpandedBrand(isOpen?null:b.id)}
              style={{display:'flex',alignItems:'center',gap:12,padding:'13px 16px',cursor:'pointer',background:isOpen?b.color+'18':'var(--card)',borderBottom:isOpen?`2px solid ${b.color}`:'none'}}>
...
function SidebarBrand({brand,sel,editor,admin,favorites,onToggleFav,onNav,onAddModel,onDelModel,onAddCat,onEditCat,onDelCat}) {
  ...
  useEffect(() => {
    if (sel?.bid === brand.id) { setOpen(true); setOpenCats(p => ({...p,[sel.cid]:true})); }
  }, [sel?.bid, sel?.cid]);
  ...
  return (
    <div style={{borderBottom:'1px solid var(--border)'}}>
      <div onClick={()=>setOpen(v=>!v)}
        style={{padding:'11px 14px',background:brand.color,color:'#fff',display:'flex',alignItems:'center',cursor:'pointer',userSelect:'none',gap:6}}>
          ...
          <div key={c.id}>
            <div style={{display:'flex',alignItems:'center',background:'var(--row2)',borderBottom:'1px solid var(--border)',minHeight:36}}>
              {editCat?.id===c.id && admin
                ...
                : <div onClick={()=>toggleCat(c.id)}
                    style={{flex:1,padding:'8px 14px 8px 20px',cursor:'pointer',color:'var(--sub)',fontSize:13,userSelect:'none',display:'flex',alignItems:'center'}}>
            ...
            {openCats[c.id] && <>
              {addingMod===c.id && (
                <div style={{padding:'6px 10px',background:'var(--row2)',display:'flex',gap:6,borderBottom:'1px solid var(--border)'}}>
                  <input ref={modRef} value={newModName} onChange={e=>setNewModName(e.target.value)}
                    onKeyDown={e=>{if(e.key==='Enter')doAddMod(c.id);if(e.key==='Escape'){setAddingMod(null);setNewModName('');}}}
                    placeholder="שם הדגם..." style={{flex:1,border:'1px solid var(--border)',borderRadius:4,padding:'5px 8px',fontSize:12,color:'var(--inp)',background:'var(--ibg)'}}/>
...
function CartPanel({cart, data, onRemove, onClear, onClose, waDefaults}) {
  ...
  const allCols = useMemo(() => {
    const s = new Set();
    cart.forEach(i => i.columns.forEach(c => s.add(JSON.stringify({id:c.id,name:c.name}))));
    return [...s].map(x => JSON.parse(x));
  }, [cart]);
        ...
        <div style={{maxHeight:'55vh',overflowY:'auto'}}>
          ...
          {techRequests.map(r => (
            <div key={r.id} style={{padding:'12px',borderRadius:8,border:`1px solid ${r.resolved?'var(--border)':'#1565c055'}`,background:r.resolved?'var(--row2)':'#e3f2fd',marginBottom:8,opacity:r.resolved?.6:1}}>
              <div style={{display:'flex',gap:8,alignItems:'flex-start'}}>
                <div style={{flex:1}}>
                  <div style={{fontWeight:'bold',fontSize:13,color:'var(--text)',marginBottom:4}}>{r.text}</div>
                  <div style={{fontSize:11,color:'var(--sub)'}}>{r.ts} · {r.submittedBy||'?'}</div>
                </div>
