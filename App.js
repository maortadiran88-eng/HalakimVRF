function App() {
  ...
  // ── Global search — shows model name + synonyms + part name + מק"ט ──
  const results = useMemo(() => {
    ...
    data.brands.forEach(b => b.categories.forEach(c => c.models.forEach(m => {
      ...
      if ((mh||ph.length) && !seen.has(m.id)) {
        seen.add(m.id);
        res.push({b,c,m,ph,ms:ms||null});
      }
    })));
    return res;
  ...
  const moveModel = (fBid,fCid,mid,toBid,toCid) => {
    ...
    mut(d => {
      const brands=d.brands.map(b => {
        if (b.id!==fBid) return b;
        return {...b,categories:b.categories.map(c => {
          if (c.id!==fCid) return c;
          moved=c.models.find(m=>m.id===mid);
  ...
  const bulkMoveModels = (sels,toBid,toCid) => {
    mut(d => {
      let moved=[];
      let brands=d.brands.map(b=>({...b,categories:b.categories.map(c=>({...c,models:c.models.filter(m=>{
        const s=sels.some(x=>x.mid===m.id&&x.bid===b.id&&x.cid===c.id);
        if(s)moved.push(m); return !s;
      })}))}));
    ...
    fbHist({action:'העברה מרובה',count:sels.length,role:loginRole});
  };
  ...
  const bulkDeleteModels = async sels => {
    ...
    fbHist({action:'מחיקה מרובה',count:sels.length,role:loginRole});
  };
  ...
  const importFromXls = (rows,colMap,tBid,tCid,excluded) => {
    ...
    mut(d => {
      const brands=d.brands.map(b => {
        if (b.id!==tBid) return b;
        return {...b,categories:b.categories.map(c => {
          if (c.id!==tCid) return c;
          let models=[...c.models];
          Object.entries(grouped).forEach(([mn,pRows]) => {
            let m=models.find(x=>x.name===mn);
            const newId=m?m.id:gid();
            if (!m) { m={id:newId,name:mn,synonyms:[],images:[],notes:'',columns:DCOLS(),parts:[]}; models.push(m); }
            const np=pRows.map(r=>({id:gid(),discontinued:false,tags:'',pinned:false,comments:[],values:{ref:'',nameHe:String(r[colMap.nameHe]||'').trim(),nameEn:String(r[colMap.nameEn]||'').trim(),mfgPn:String(r[colMap.mfgPn]||'').trim(),tadPn:String(r[colMap.tadPn]||'').trim()}}));
            total+=np.length; changedMids.current.add(newId);
            models=models.map(x=>x.name===mn?{...x,parts:[...x.parts,...np]}:x);
          });
          return {...c,models};
        })};
      });
  ...
  // ── Excel export — every model gets its own sheet + "כל הנתונים" ──
  const expXLS = () => {
    if (!data) return;
    ...
    data.brands.forEach(b => b.categories.forEach(c => c.models.forEach(m => {
      ...
      const rows = m.parts.map(p => [
        p.values.ref||'', p.values.nameHe||'', p.values.nameEn||'', p.values.mfgPn||'', p.values.tadPn||'',
        ...extraCols.map(c=>p.values[c.id]||''),
        p.discontinued ? 'הופסק לייצור' : ''
      ]);
      const sheetName = `${b.name}-${m.name}`.slice(0,31);
      ...
      m.parts.forEach(p => allRows.push([
        b.name, c.name, m.name,
  ...
  const impFile = e => {
    ...
    r.onload=ev=>{
      try {
        ...
        d.brands?.forEach(b=>b.categories?.forEach(c=>c.models?.forEach(m=>changedMids.current.add(m.id))));
        setData(d); setSel(null); alert('✅ נטען');
      } catch { alert('❌ שגיאה'); }
    };
    r.readAsText(f); e.target.value='';
  };

  // When a new model is added via sidebar, auto-post news
  const handleAddModel = (b, cid, name) => {
    ...
    fbHist({action:'הוסף דגם',model:name,brand:b.name,role:loginRole});
  ...
  return (
    <div dir="rtl" style={{fontFamily:'Arial,sans-serif',minHeight:'100vh',background:'var(--bg)',display:'flex',flexDirection:'column',fontSize:14,color:'var(--text)'}}>
              ...
              {r.ph.slice(0,3).map(p => {
                ...
                return (
                  <div key={p.id} style={{fontSize:11,color:'var(--sub)',paddingRight:8,marginBottom:2,display:'flex',gap:10,alignItems:'center',flexWrap:'wrap'}}>
                    {p.discontinued && (
                      <span style={{background:'#e53935',color:'#fff',borderRadius:4,padding:'1px 6px',fontSize:10,fontWeight:'bold'}}>⛔ הופסק</span>
                    )}
      ...
      <div style={{display:'flex',flex:1,overflow:'hidden',height:'calc(100vh - 56px)'}}>
        ...
        {/* Sidebar — ALL roles see it, toggle for editor/admin, always open for viewer */}
        <aside style={{
          width: viewer ? 240 : (sidebar ? 265 : 0),
          flexShrink:0,overflow:'hidden',
          transition:'width .25s',
          background:'var(--sidebar)',
          borderLeft:'1px solid var(--border)'
        }}>
          <div style={{width: viewer ? 240 : 265, overflowY:'auto',height:'100%'}}>
            {data.brands.map(b => (
              <SidebarBrand key={b.id} brand={b} sel={sel} editor={editor} admin={admin} favorites={favorites}
                onToggleFav={toggleFav} onNav={nav}
        ...
        <main style={{flex:1,overflowY:'auto',padding:14}}>
          {!model
            ? <HomeScreen data={data} onNav={nav} recent={recent} favorites={favorites} onToggleFav={toggleFav} loginRole={loginRole}/>
            : <ModelView
      ...
      {showHistory && (
        <Modal onClose={()=>setShowHistory(false)} title="📋 לוג שינויים" wide>
      ...
      {chPwd      && <ChangePwd     data={data} onSave={d=>{mut(()=>({...d}));setChPwd(false);}} onClose={()=>setChPwd(false)}/>}
